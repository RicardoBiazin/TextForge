...
| `grande/edicao.py` | A pilha de desfazer guarda **operações inversas**, nunca um instantâneo da lista de trechos. A primeira versão guardava `list(self._trechos)` por operação: 10 mil edições num arquivo de 59 MB **mediram 800 MB** de RAM, porque cada cópia carrega os trechos que as edições anteriores criaram. |
| `grande/edicao.py` | Há **no máximo um trecho ABERTO, e sempre no fim**. É essa invariante que deixa editar durante a indexação: a thread de disco só faz `append`, o total de linhas cresce, e o trecho aberto absorve isso sozinho. Partir um aberto tem de produzir prefixo fechado + sufixo aberto. |
| `grande/gravacao.py` | O mmap fica **aberto** durante a escrita (é de onde vêm os trechos intocados) e é fechado na emenda `antes_de_trocar`, imediatamente antes da troca atômica. Fechar antes grava arquivo vazio; deixar aberto faz a troca falhar com acesso negado. |
| `documento.py` | `bytes_para_salvar()` **recusa** arquivo grande. Ele leria o `QTextDocument`, que em modo grande fica vazio de propósito — era assim que "Salvar como" gravava um arquivo de zero byte por cima do original. |
| `arquivos.py` | Acima de 8 MB não há sha256, e só tamanho + mtime **não detecta** uma reescrita externa que preserve os dois. Por isso existe a assinatura por amostra das pontas, pedida com `amostrar=True` **só no caminho de gravação** — o vigia roda a cada 2 s e não pode ler 2 MB por arquivo nesse ritmo. |
| `sessao.py` | Arquivo grande modificado grava **diário** (`.diario`, alguns KB), nunca cópia de conteúdo. Copiar 240 MB para `%APPDATA%` a cada autosave seria pior que não ter recuperação. |
